...
impl JavaDownloader {
    ...
    pub async fn get_download_url(
        ...
        // 镜像源 URL
        let image_type = java_type.as_str().to_uppercase(); // JRE, JDK
        let mut page_url = format!(
            "https://mirrors.tuna.tsinghua.edu.cn/Adoptium/{}/{}/{}",
            feature_version, image_type, os
        );

        // 添加架构子路径
        page_url = format!("{}/{}", page_url, arch);
    ...
    pub async fn download_java(
        ...
        let url = self.get_download_url(version, java_type).await?;
