...
use crate::java::{JavaDownloader, JavaType};
use crate::modloader::client_json;
use crate::modloader::fabric::FabricInstaller;
use crate::modloader::labymod::LabyMod4Installer;
use crate::types::*;
...
fn get_version_list(
    ...
    show_release: bool,
    bmclapi: bool,
    show_old_alpha: Option<bool>,
    show_old_beta: Option<bool>,
) -> PyResult<Vec<String>> {
    init(bmclapi)?;
    let old_alpha = show_old_alpha.unwrap_or(show_old);
    let old_beta = show_old_beta.unwrap_or(show_old);
    let client = http().clone();
    run_async(py, async move {
        let mut eng = DownloadEngine::new(client);
        eng.get_version_list(show_snapshot, old_alpha, old_beta, show_release).await
    })
}
...
#[pyfunction]
fn get_architecture_py() -> &'static str {
    get_architecture()
}

#[pyfunction]
fn get_system_bits_py() -> &'static str {
    get_system_bits()
}

#[pyfunction]
fn get_architecture_key_py() -> String {
    get_architecture_key()
}

#[pyfunction]
#[pyo3(signature = (java_major_version, java_type="jre", tuna=true))]
fn get_java_download_url(
    py: Python<'_>,
    java_major_version: i32,
    java_type: &str,
    tuna: bool,
) -> PyResult<String> {
    init(true)?;
    let jt = match java_type.to_lowercase().as_str() {
        "jdk" => JavaType::Jdk,
        _ => JavaType::Jre,
    };
    let client = http().clone();
    run_async(py, async move {
        JavaDownloader::new(client)
            .get_download_url(java_major_version, jt, tuna)
            .await
    })
}

#[pyfunction]
fn get_fabric_versions(py: Python<'_>) -> PyResult<Vec<String>> {
    init(true)?;
    let client = http().clone();
    run_async(py, async move {
        FabricInstaller::new(client).get_all_loader_versions().await
    })
}

#[pyfunction]
#[pyo3(signature = (minecraft_dir, mcversion, instance_name, mod_version=None))]
fn download_fabric_api(
    py: Python<'_>,
    minecraft_dir: &str,
    mcversion: &str,
    instance_name: &str,
    ...
    let mcversion = mcversion.to_string();
    let instance_name = instance_name.to_string();
    let mod_version = mod_version.map(String::from);
    run_async(py, async move {
        FabricInstaller::new(client)
            .download_fabric_api(
                Path::new(&minecraft_dir),
                &mcversion,
                &instance_name,
...
#[pyfunction]
fn get_labymod_versions(py: Python<'_>) -> PyResult<Vec<String>> {
    init(true)?;
    let client = http().clone();
    run_async(py, async move { LabyMod4Installer::new(client).get_versions().await })
}

#[pyfunction]
#[pyo3(signature = (minecraft_directory, version, mcversion, instance_name))]
fn labymod_download(
    py: Python<'_>,
    ...
    init(true)?;
    let client = http().clone();
    let minecraft_directory = minecraft_directory.to_string();
    let mcversion = mcversion.to_string();
    let instance_name = instance_name.to_string();
    run_async(py, async move {
        LabyMod4Installer::new(client)
            .download(
                Path::new(&minecraft_directory),
                version,
                &mcversion,
                &instance_name,
            )
...
fn download_forge_json(
    ...
    let mcversion = mcversion.to_string();
    let instance_name = instance_name.to_string();
    let forge_version = forge_version.map(String::from);
    let java = java.to_string();
    run_async(py, async move {
        client_json::download_forge_json(
            client,
            Path::new(&minecraft_dir),
            &mcversion,
            &instance_name,
...
fn download_neoforge_json(
    ...
    let mcversion = mcversion.to_string();
    let instance_name = instance_name.to_string();
    let neoforge_version = neoforge_version.map(String::from);
    let java = java.to_string();
    run_async(py, async move {
        client_json::download_neoforge_json(
            client,
            Path::new(&minecraft_dir),
            &mcversion,
            &instance_name,
...
pub fn init_module(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(get_architecture_py, m)?)?;
    m.add_function(wrap_pyfunction!(get_system_bits_py, m)?)?;
    m.add_function(wrap_pyfunction!(get_architecture_key_py, m)?)?;
    m.add_function(wrap_pyfunction!(get_java_download_url, m)?)?;
    m.add_function(wrap_pyfunction!(get_fabric_versions, m)?)?;
    m.add_function(wrap_pyfunction!(download_fabric_api, m)?)?;
    m.add_function(wrap_pyfunction!(get_labymod_versions, m)?)?;
    m.add_function(wrap_pyfunction!(labymod_download, m)?)?;
    m.add_function(wrap_pyfunction!(download_forge_json, m)?)?;
    m.add_function(wrap_pyfunction!(download_neoforge_json, m)?)?;
