...
impl HttpClient {
    ...
    /// 带自定义请求头的 GET JSON（如 GitHub API）
    pub async fn get_json_with_headers<T: serde::de::DeserializeOwned + Send>(
        &self,
        url: &str,
        headers: &[(&str, &str)],
    ) -> CoreResult<T> {
        self.get_with_headers_retry(url, headers, |resp| resp.json::<T>())
            .await
    }
    ...
    where
        ...
        T: Send,
    {
        self.get_with_headers_retry(url, &[], extractor).await
    }

    async fn get_with_headers_retry<F, Fut, T>(
        &self,
        url: &str,
        headers: &[(&str, &str)],
        extractor: F,
    ...
    /// 规则参考 BMCL Mirrors: https://github.com/bangbang93/BMCL/tree/master/BMCLV2/Mirrors
    pub fn resolve_url(&self, url: &str) -> String {
        if !self.bmclapi {
            return url.to_string();
        }

        const BMCL: &str = "https://bmclapi2.bangbang93.com";

        // 顺序：先替换更长、更具体的路径，避免被通用规则截断
        url.replace(
            "https://maven.neoforged.net/releases/net/neoforged/neoforge",
            &format!("{BMCL}/maven/net/neoforged/neoforge"),
        )
        .replace(
            "https://maven.neoforged.net/releases/net/neoforged/forge",
            &format!("{BMCL}/maven/net/neoforged/forge"),
        )
        .replace("https://launchermeta.mojang.com", BMCL)
        .replace("http://launchermeta.mojang.com", BMCL)
        .replace("https://launcher.mojang.com", BMCL)
        .replace("http://launcher.mojang.com", BMCL)
        .replace("https://piston-meta.mojang.com", BMCL)
        .replace("https://piston-data.mojang.com", BMCL)
        .replace(
        ...
        .replace("https://libraries.minecraft.net/", &format!("{BMCL}/maven/"))
        .replace("https://libraries.minecraft.net", &format!("{BMCL}/maven"))
        .replace("http://libraries.minecraft.net/", &format!("{BMCL}/maven/"))
        .replace("http://libraries.minecraft.net", &format!("{BMCL}/maven"))
        .replace(
            "https://files.minecraftforge.net/maven",
            &format!("{BMCL}/maven"),
        )
        .replace("https://maven.minecraftforge.net", &format!("{BMCL}/maven"))
        .replace("https://maven.fabricmc.net", &format!("{BMCL}/maven"))
        .replace("https://meta.fabricmc.net", &format!("{BMCL}/fabric-meta"))
        .replace("https://maven.neoforged.net", &format!("{BMCL}/maven"))
        .replace(
        ...
        .replace(
            ...
            &format!("{BMCL}/mirrors/authlib-injector"),
        )
        .replace(
            "https://maven.quiltmc.org/repository/release",
            &format!("{BMCL}/maven"),
        )
        .replace("https://meta.quiltmc.org", &format!("{BMCL}/quilt-meta"))
    }
...
mod tests {
    ...
    fn test_url_resolve() {
        ...
        assert_eq!(
            client.resolve_url("https://libraries.minecraft.net/org/lwjgl/lwjgl/3.3.1/lwjgl-3.3.1.jar"),
            "https://bmclapi2.bangbang93.com/maven/org/lwjgl/lwjgl/3.3.1/lwjgl-3.3.1.jar"
        );
        assert_eq!(
            client.resolve_url("https://resources.download.minecraft.net/ab/cd/file"),
            "https://bmclapi2.bangbang93.com/assets/ab/cd/file"
        );
    }
}
