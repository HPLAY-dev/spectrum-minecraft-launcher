...
def fabric_merge_json(path_a, path_b, modify=True) -> dict:
    ...
    for i in range(len(json_b['libraries'])):
        if json_b['libraries'][i][
            'name'] == 'org.ow2.asm:asm:9.6':  # 我怎么感觉这么写会出事呢awa�?本意是要丢掉minecraft版本json里与fabric重复的asm项，有空(也不一�?改，（主打一个能跑就行awa�?
...
def download_fabric_json(minecraft_dir, mcversion, instance_name, loader_version='latest') -> None:
    '''下载Fabric-Loader的JSON文件�?minecraft/versions/{instance_name}/Fabric.json，用于和当前版本的Minecraft的json合并'''
