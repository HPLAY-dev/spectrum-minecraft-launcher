...
from spectrum_core.py_fallback.manifest_funcs import get_version_json
...
def download_forge_json(minecraft_dir, mcversion, instance_name, forge_version='latest', bmclapi=False, java='java'):
    """下载一个包含了Forge的东西的版本json，附赠一份forge client jar于Libraries(其实无非就是调用安装器罢�?"""
    ...
    def deep_merge(a, b):
        ...
        if isinstance(a, list) and isinstance(b, list):
            merged = []
            seen = set()
            # �?b（来�?Forge 的条目）先加入，保持 Forge 条目靠前
            for item in b:
                merged.append(item)
            ...
            for item in a:
                ...
                seen.add(key)
            return merged
        # 对于标量或类型不匹配，优先保�?a（原版数据）以避免覆盖已存在设置
