...
from spectrum_core.py_fallback.launcher_funcs import get_version_manifest, get_assetIndex, get_minecraft_libraries, is_library_required
from concurrent.futures import ThreadPoolExecutor, as_completed

from spectrum_core.py_fallback.modloader_fabric import download_fabric_json, fabric_merge_json
from spectrum_core.py_fallback.modloader_forge import download_forge_json
from spectrum_core.py_fallback.modloader_neoforge import download_neoforge_json
from spectrum_core.py_fallback.tool_funcs import *
...
def get_version_list(show_snapshot=False, show_old=False, show_release=True, bmclapi=False) -> list:
    ...
    if os.path.exists('version_manifest.temp'):
        ...
        if current_time - file_time < 240:  # 240�?= 4分钟
...
def download_libraries(minecraft_dir, mcversion, instance_name, print_status=True, bmclapi=False, progress_callback=None,
                       max_workers=5):
    """下载Minecraft为指定版本的libraries(�?，返回None"""
    with open(f'{minecraft_dir}/versions/{instance_name}/{instance_name}.json', 'r') as f:
        version_json = f.read()
