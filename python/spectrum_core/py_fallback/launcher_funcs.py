from spectrum_core.py_fallback.oauth_funcs import get_mc_token, get_mslogin_uuid_name
from spectrum_core.py_fallback.tool_funcs import *
from spectrum_core.py_fallback.java import get_java_version
from spectrum_core.py_fallback.modloader_fabric import is_fabric
from spectrum_core.py_fallback.tool_funcs import maven_to_path
...
def check_java_available(java_binary_path, minecraft_dir, instance_name) -> bool:
    """查看java是否符合要求 (不在launch()中使�?, 返回bool"""
...
def get_mainclass(minecraft_dir, instance_name) -> str:
    """获取Minecraft版本的mainClass，Fabric�?net.fabricmc.loader.impl.launch.knot.KnotClient'，返回str"""
...
def get_minecraft_libraries(minecraft_dir, instance_name, detailed=False) -> list:
    """获取minecraft的所有需要libraries(�?，返回list"""
    # read json
    version_json_path = f'{minecraft_dir}/versions/{instance_name}/{instance_name}.json'
