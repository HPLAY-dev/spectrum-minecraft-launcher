...
class WebBridge(QObject):
    ...
    def searchModrinth(self, query: str, loader: str) -> str:
        self._backend.lineEdit_8.setText(query)
        idx = self._backend.comboBox_3.findText(loader)
        if idx >= 0:
            self._backend.comboBox_3.setCurrentIndex(idx)
        self._backend.search_modrinth()
        return json.dumps(
            [{"index": i, "id": mid} for i, mid in enumerate(getattr(self._backend, "mods", []))],
            ensure_ascii=False,
        )
    ...
    def installMod(self, index: int) -> bool:
        try:
            ...
            model = self._backend.listView_3.model()
            if model and 0 <= index < model.rowCount():
                ix = model.index(index, 0)
                sel = QItemSelection(ix, ix)
                self._backend.listView_3.selectionModel().select(
                    sel, QItemSelectionModel.SelectionFlag.ClearAndSelect
                )
            self._backend.install_modrinth()
            return True
        except Exception:
            return False
